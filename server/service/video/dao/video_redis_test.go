...
func TestVideoLifecycleInRedis(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			op: func() (string, error) {
				for _, v := range videoList {
					if err = manager.PublishVideo(ctx, v); err != nil {
						return "", err
					}
				}
				return "", nil
		...
		},
		{
			...
			op: func() (string, error) {
				err = manager.PublishVideo(ctx, videoList[0])
				return "", err
			},
			wantErr: true,
		},
		{
			...
			op: func() (string, error) {
				videos, err := manager.GetBasicVideoListByLatestTime(ctx, videoList[0].AuthorId, timeStamp+2)
				if err != nil {
					return "", err
				}
		...
		},
		{
			...
			op: func() (string, error) {
				vidList, err := manager.GetPublishedVideoListByUserId(ctx, videoList[0].AuthorId)
				if err != nil {
					return "", err
				}
